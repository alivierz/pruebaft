...
export class EnrollmentProcessor implements FaceTecFaceScanProcessor {
  ...
  callbackData: any;
  enrrollementToken: any;
  constructor(sessionToken: string, sampleAppControllerReference: any , dataCallback: (sessionToken?: string)=>void, enrrollementToken: any) {
    //
    // DEVELOPER NOTE:  These properties are for demonstration purposes only so the Sample App can get information about what is happening in the processor.
    // In the code in your own App, you can pass around signals, flags, intermediates, and results however you would like.
    //
    ...
    this.enrrollementToken = enrrollementToken
    //
  ...
  public processSessionResultWhileFaceTecSDKWaits = (sessionResult: FaceTecSessionResult, faceScanResultCallback: FaceTecFaceScanResultCallback): void => {
    ...
    var parameters = {
      ...
      externalDatabaseRefID: this.enrrollementToken
    };
  ...
  public onFaceTecSDKCompletelyDone = (): void => {
    ...
    this.callbackData({value: this.latestSessionResult, valuedos: null,value3: this.latestNetworkRequest.status})
  };
